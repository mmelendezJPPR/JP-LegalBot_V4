...
def ensure_schema_for_db(path_or_url: str) -> dict:
    """Ensure the expected tables and columns exist in the given sqlite db path.

    Returns a dict with status and messages for diagnostics.
    """
    from pathlib import Path
    from urllib.parse import urlparse
    messages = []
    try:
        p = path_or_url
        # Normalize sqlite:/// urls -> filesystem path
        try:
            parsed = urlparse(p)
            if parsed.scheme and parsed.scheme.startswith('sqlite'):
                if p.startswith('sqlite:///'):
                    p = p.replace('sqlite:///', '', 1)
        ...
        p = Path(p)
        if not p.exists():
            # Create parent and touch file so sqlite will create it
            p.parent.mkdir(parents=True, exist_ok=True)
            conn = sqlite3.connect(str(p))
            conn.close()
            messages.append(f"Created sqlite file: {p}")
        ...
        ''')
        messages.append('ensured table: conversaciones')

        # conversation_messages (may already exist but missing specialist_context)
        cur.execute('''
            CREATE TABLE IF NOT EXISTS conversation_messages (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                conversation_id TEXT,
        ...
        ''')
        # add specialist_context column if missing
        try:
            cols = [r[1] for r in cur.execute("PRAGMA table_info(conversation_messages)")]
            if 'specialist_context' not in cols:
                cur.execute('ALTER TABLE conversation_messages ADD COLUMN specialist_context TEXT')
                messages.append('added column: conversation_messages.specialist_context')
        except Exception as e:
            messages.append(f'warning: could not ensure specialist_context: {e}')

        # performance_metrics ensure metric_type
        cur.execute('''
            CREATE TABLE IF NOT EXISTS performance_metrics (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,
        ...
        ''')
        try:
            cols = [r[1] for r in cur.execute("PRAGMA table_info(performance_metrics)")]
            if 'metric_type' not in cols:
                cur.execute('ALTER TABLE performance_metrics ADD COLUMN metric_type TEXT')
                messages.append('added column: performance_metrics.metric_type')
        except Exception as e:
            messages.append(f'warning: could not ensure metric_type: {e}')

        # FTS virtual table
        try:
            cur.execute("CREATE VIRTUAL TABLE IF NOT EXISTS fts_chunks USING fts5(id, content, doc_id, heading_path, page_start, page_end, chunk_text)")
            messages.append('ensured virtual table: fts_chunks')
        except Exception as e:
            messages.append(f'warning: could not create fts_chunks (FTS5 available?): {e}')

        conn.commit()
        conn.close()
        return {'ok': True, 'messages': messages, 'path': str(p)}
    except Exception as e:
        return {'ok': False, 'error': str(e), 'messages': messages}


app = Flask(__name__)


@app.route('/ensure-schema', methods=['POST', 'GET'])
def ensure_schema_endpoint():
    """HTTP endpoint to ensure schema exists in both conversation and hybrid DBs.

    GET: returns diagnostic info without making changes (if ?apply=1 then apply)
    POST: forces apply
    """
    # Basic protection: require ADMIN_SCHEMA_TOKEN env var when applying changes
    apply_changes = request.args.get('apply') == '1' or request.method == 'POST'
    admin_token = os.getenv('ADMIN_SCHEMA_TOKEN')
    if apply_changes and admin_token:
        # token may be passed as header X-ADMIN-TOKEN or query ?token=
        provided = request.headers.get('X-ADMIN-TOKEN') or request.args.get('token')
        if not provided or provided != admin_token:
            return jsonify({'ok': False, 'error': 'admin token required or invalid'}), 403
    conv = os.getenv('CONVERSACIONES_DB') or os.getenv('DATABASE_URL') or 'database/conversaciones.db'
    hybrid = os.getenv('DB_PATH') or os.getenv('DATABASE_URL') or 'database/hybrid_knowledge.db'

    results = {'conversaciones': None, 'hybrid': None, 'applied': apply_changes}
    if apply_changes:
        results['conversaciones'] = ensure_schema_for_db(conv)
        results['hybrid'] = ensure_schema_for_db(hybrid)
    else:
        # just report resolved paths
        results['conversaciones'] = {'path': conv}
        results['hybrid'] = {'path': hybrid}

    return jsonify(results)


# Optional: automatically ensure schema at startup if environment variable enabled.
try:
    if os.getenv('ENSURE_SCHEMA_ON_STARTUP', '0') == '1':
        logger.info('ENSURE_SCHEMA_ON_STARTUP=1 -> ensuring schema on startup')
        conv_path = os.getenv('CONVERSACIONES_DB') or os.getenv('DATABASE_URL') or 'database/conversaciones.db'
        hybrid_path = os.getenv('DB_PATH') or os.getenv('DATABASE_URL') or 'database/hybrid_knowledge.db'
        r1 = ensure_schema_for_db(conv_path)
        r2 = ensure_schema_for_db(hybrid_path)
        logger.info('Startup schema ensure results: %s %s', r1.get('messages') if isinstance(r1, dict) else r1, r2.get('messages') if isinstance(r2, dict) else r2)
except Exception as e:
    logger.exception('Error ensuring schema on startup: %s', e)
...
def health_check():
    ...
    try:
        ...
        for db_path in ['database/conversaciones.db', 'database/hybrid_knowledge.db', 'database/Usuarios.db']:
            try:
                conn = sqlite3.connect(db_path)
        ...
        # Verificar que Azure OpenAI está configurado
        azure_ok = bool(os.getenv('AZURE_OPENAI_ENDPOINT') and os.getenv('AZURE_OPENAI_KEY'))
        
        status = "healthy" if dbs_ok and azure_ok else "unhealthy"
        
        return jsonify({
            "status": status,
            "timestamp": datetime.now().isoformat(),
            "databases": "ok" if dbs_ok else "error",
